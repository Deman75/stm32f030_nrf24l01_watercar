...
extern uint8_t SRV_1_C, SRV_1_Left, SRV_1_Right;
...
void NRF24_Write_Buf(uint8_t addr,uint8_t *pBuf,uint8_t bytes)
{
  ...
  DelayMicro(1);
	for (i=0; i<bytes; i++) {
		HAL_SPI_Transmit(&hspi1,&pBuf[i],1,1000);//ioi?aaei aaiiua a aooa?
	}
...
void NRF24L01_Receive(void)
{
  if(rx_flag==1)
  {
		...
		if (RX_BUF[4] == 2){
			...
			buf1[4] = 2;
			NRF24L01_Send(buf1);
			if (RX_BUF[0]>30 && RX_BUF[0]<SRV_1_C)
				SRV_1_Left = RX_BUF[0];
			Servo1 = SRV_1_C - (SRV_1_C-SRV_1_Left);	
			HAL_Delay(10);
			TimeLastRx = 0;
		}
		if (RX_BUF[4] == 3){
			...
			buf1[4] = 3;
			NRF24L01_Send(buf1);
			if (RX_BUF[0]>SRV_1_C && RX_BUF[0]<200)
				SRV_1_Right = RX_BUF[0];
			Servo1 = SRV_1_C + (SRV_1_Right-SRV_1_C);	
			HAL_Delay(10);	
			TimeLastRx = 0;
		}
		if (RX_BUF[4] == 5){
			...
			buf1[4] = 5;
			NRF24L01_Send(buf1);
			if (RX_BUF[0]>SRV_1_Left && RX_BUF[0]<SRV_1_Right)
				SRV_1_C = RX_BUF[0];
			Servo1 = SRV_1_C;	
			HAL_Delay(10);
			TimeLastRx = 0;			
